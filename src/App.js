...
const GlobeWithLabels = () => {
  ...
  // Alternative method: Manual rotation with custom animation
  // Uncomment this and comment out the useEffect above to use manual rotation
  /*
  useEffect(() => {
    if (globeRef.current) {
      let angle = 0;
      const animate = () => {
        if (globeRef.current) {
          ...
          const lat = 0; // Keep latitude at equator
          const lng = angle;
          globeRef.current.pointOfView({ lat, lng, altitude: 2 });
        }
  ...
  return (
    <div style={{ width: '100%', height: '500px' }}>
      <Globe
        ...
        // Optional: Disable user controls if you want pure auto-rotation
